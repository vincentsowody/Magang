        ...
        <div style="display:flex;flex-direction:column;gap:14px">
            ...
            <div class="panel">
                ...
                <div style="padding:16px">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
                        <div>
                            ...
                            <select id="doc-type" class="form-select">
                                ...
                                <option value="surat_pengantar">Surat Pengantar Kampus</option>
                                <option value="lainnya">Dokumen Lainnya</option>
                            </select>
                        </div>
                        <div>
                            <label class="form-label">Nama Dokumen (opsional)</label>
                            <input type="text" id="doc-name" class="form-input" placeholder="e.g. CV Terbaru 2025">
                        </div>
                    </div>
                    ...
                    <div id="file-preview" style="display:none;margin-top:10px;padding:10px 12px;background:rgba(59,130,246,0.06);border:1px solid rgba(59,130,246,0.15);border-radius:10px;display:none;align-items:center;justify-content:space-between">
...
async function loadAdminDocs() {
    // Ambil dokumen admin sesuai lokasi dari localStorage atau API
    const loc = currentUser?.location;
    const el  = document.getElementById('admin-doc-list');
    if (!loc) { el.innerHTML = '<div style="font-size:12px;color:var(--muted)">Penempatan belum ditentukan.</div>'; return; }
    // Dummy — dalam implementasi nyata fetch dari /api/location-docs?loc=...
    const dummies = [
        {name:'Panduan Orientasi PKL', type:'PDF'},
        {name:'Formulir Absensi', type:'PDF'},
        {name:'SOP ' + (loc === 'kantor' ? 'Kantor Pusat' : 'Terminal Ops'), type:'DOC'},
    ];
    el.innerHTML = dummies.map(d => `
        <div class="doc-row" style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.04)">
            <div style="flex:1;font-size:12px;color:rgba(148,163,184,.8)">${d.name}</div>
