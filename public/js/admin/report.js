            ...
            async function loadReport() {
                try {
                    const res = await fetch(API_BASE_URL + '/report/summary');
    ...
    .map(([univ, v]) => ({
        ...
        ...v
            ...
            function renderReportOverview(ov) {
                ...
                set('rep-total',    ov.total    ?? 0);
                set('rep-pending',  ov.pending  ?? 0);
                set('rep-accepted', ov.accepted ?? 0);
                set('rep-rejected', ov.rejected ?? 0);
                set('rep-kantor',   ov.kantor   ?? 0);
                set('rep-terminal', ov.terminal ?? 0);
            }
            ...
            function renderDonut(ov) {
                ...
                if (accEl) { accEl.setAttribute('stroke-dasharray', `${aLen} ${CIRC - aLen}`); accEl.setAttribute('stroke-dashoffset', CIRC * 0.25); }
                if (rejEl) { rejEl.setAttribute('stroke-dasharray', `${rLen} ${CIRC - rLen}`); rejEl.setAttribute('stroke-dashoffset', CIRC * 0.25 - aLen); }
                if (penEl) { penEl.setAttribute('stroke-dasharray', `${pLen} ${CIRC - pLen}`); penEl.setAttribute('stroke-dashoffset', CIRC * 0.25 - aLen - rLen); }
            ...
            function renderUnivTable(byUniv) {
                ...
                if (!tbody) return;
                if (count) count.textContent = (byUniv?.length || 0) + ' universitas';
